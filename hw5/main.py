import math
import config
from hwtwo import get_tokens, get_lemmas
from hwf import analyse
from search_inverted_index import get_inverted_index

inverted_idx = get_inverted_index()
tf_idf_dicts_lemmas, idf_lemmas = analyse(config.LEMMAS_FILE_NAME, config.LEMMAS_PATH)
ALL_DOCS_COUNT = 100
...
def calculate(term, document_tokens_list, documents_count, documents_with_term_count):
    ...
    tf = len(document_tokens_list[term]) / len(document_tokens_list)
    print(len(document_tokens_list[term]))
